...
import { user } from '@/tests/fixtures/user.fixture';
...
describe('Store Routes', () => {
	...
	describe('PATCH /store', () => {
		it('should return store with 200 status ', async () => {
			...
			const res = await request(app).patch(`/stores/${resInsert.body._id}`).set('Authorization', `bearer ${token}`).send({ store: updateStore }).expect(200);
